...
def AIvalidate(mode: str, payload: dict | None = None, **kwargs) -> bool:
    ...
    try:
        import re
        import openai
        from config import key, base_url, model_validation, validation_message
...
"""
    ...
    print("[AIconversation] rejected 3 times -> returning '' to trigger parsing fallback")
    return ""
...
def get_story(player_input: str):
    ...
    # ------------------------------------------------------------------
    # 7) End-of-turn knowledge refresh (explicitly requested)
    # ------------------------------------------------------------------
    try:
        p = getattr(gameSetup, "player", None)
        if p is not None:
