...
model_precheck = "openai/gpt-4.1"
model_undo = "openai/gpt-4.1i"
model_parsing = "openai/gpt-4.1"
model_talking = "openai/gpt-4.1"
model_storytelling = "openai/gpt-4.1"
...
Long: The message contains more than 50 words and more than 3 actions
    Example: 
    Ok so I go to the pharmacy right, I dig through all the stuff until I find a medkit, I go back and I heal Larry
    I tell Carley to fuck off, Kenny to come with me and leave his wife, Clem to go to the storage room and get some medicine for us while we barricade the front door
...
Unrelated: The message does not have anything to do with the game
    Example:
    I don't actually need to do anything
    My mom makes the best fish cakes
Impossible: The message contains an action that cannot physically be performed by a human in a zombie apocalypse
    Example:
    I cure the zombie apocalypse
    I heal Larry with my Teresa touch
...
Undo: The message indicates a request to undo the previous action
    Example:
    Oh shit wait I didnt meet to do that
    I want to go back before I shot her
Clear: The message clearly shows the players intent to undertake an action or to roleplay in the game
    I ask Carley if she has a gun
    Lets go to the storage room
"""
...
Examples of when to give an action and when to do_nothing:
    Player input: I go outside and fight the zombies
    Return: "1. action:move,requested_action:0,target:0,indirect_target:0,item:0,location:front_area
    2. action:harm,requested_action:0,target:angry zombie,indirect_target:0,item:flashlight,location:0
    3. action:harm,requested_action:0,target:ugly zombie,indirect_target:0,item:flashlightX,location:0"

    Player input: I try to convince the zombies to become friendly
    Return: "action:do_nothing,requested_action:0,target:0,indirect_target:0,item:0,location:0"

    Player input: I ask Carley how she's doing
    Return: "action:"talk",requested_action:0,target:Carley,indirect_target:0,item:0,location:0"

    Player input: I run far away and escape 
    Return "action:do_nothing,requested_action:0,target:0,indirect_target:0,item:0,location:0"

    Player input: I use the medkit on Larry
    Return: "action:use_item,requested_action:0,target:Larry,indirect_target:0,item:0,location:0"
"""
}
...
Examples of desired output:
    ...
    Return: Right, guess that wasn't too obvious, huh? You're standing in the storage room, picture a flickering light, shelves with all kinds of supplies, and two doors, one to the main store, one to the pharmacy.

    Player input: This game doesn't make a lot of sense
    Return: Haha, is it really that bad? Tell me, what's bothering you?

    Player input: (Silence)
    Return: Helloooooo, anyone still there? Need some help?

    Player input: I call my friend Kim Jong Un and nuke all the zombies
    Return: Yeah, nice try, but I'm not making it that easy for you pal!

    Player input: You're a dumb fucking bitch
    Return: I uh.... I don't really know how to respond to that.
...
You are in here with other people trying to survive as well, but one guy is hurt badly: Larry.
You win this session by finding a way of healing him.
Now, if you feel ready to start, type something out and press enter! 
""")